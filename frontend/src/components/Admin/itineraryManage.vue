        ...
        <tbody>
          <tr v-for="itinerary in itineraries" :key="itinerary.itinerary_id">
            ...
            <td class="actions-cell">
              ...
              <button @click="confirmDelete(itinerary.itinerary_id)" class="btn-danger">Delete</button>
            </td>
          </tr>
        </tbody>
      </table>
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      try {
        ...
        console.error(err)
        this.error = (err.response && (err.response.data?.error || err.response.data?.message)) 
                     || 'Operation failed. Please check your input for any time conflicts.'
      }
    },
    async confirmDelete(itinerary_id) {
      if (confirm('Are you sure you want to delete this itinerary?')) {
        this.deleteItinerary(itinerary_id)
      }
    },
